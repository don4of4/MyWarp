...
public class PropertiesFile {
    ...
    public PropertiesFile(File file) {
        ...
        try {
        	...
        	WarpLogger.severe("Cannot read file " + file.getName());
        } catch (IOException e) {
        	WarpLogger.severe("Cannot create file " + file.getName());
        }
    }
    ...
    public int getInt(String key, Integer defaultValue, String defaultComment) {
        if (map.containsKey(key)) {
            try {
            	...
            	WarpLogger.warning("Trying to get Integer from " + key + ": " + map.get(key).value);
                return 0;
            }
        } else {
            map.put(key, new PropertiesEntry(defaultValue.toString(), defaultComment));
    ...
    public long getLong(String key, Long defaultValue, String defaultComment) {
        if (map.containsKey(key)) {
            try {
            	...
            	WarpLogger.warning("Trying to get Long from " + key + ": " + map.get(key).value);
                return 0;
            }
        } else {
            map.put(key, new PropertiesEntry(defaultValue.toString(), defaultComment));
    ...
    public double getDouble(String key, Double defaultValue, String defaultComment) {
        if (map.containsKey(key)) {
            try {
            	...
            	WarpLogger.warning("Trying to get Double from " + key + ": " + map.get(key).value);
                return 0;
            }
        } else {
            map.put(key, new PropertiesEntry(defaultValue.toString(), defaultComment));
    ...
    public void save() {
        ...
        try {
        	...
        	WarpLogger.severe("IO Exception with file " + file.getName());
        } finally {
            try {
                if (bwriter != null) {
                    bwriter.flush();
            	...
            	WarpLogger.severe("IO Exception with file " + file.getName() + " (on close)");
            }
        }
